...
export async function ResetUserPassword(password, code) {
  const response = await axios.post(`${Api}/new_user_password`, {
    password,
    code,
  });
  return response;
...
export async function UpdateAdditionalUserDetails(country, company) {
  const access_token = localStorage.getItem('access_token');
  try {
    const response = await axios.post(
      `${Api}/update_additional_profile`,
      { country, company },
      {
        headers: {
          Authorization: `Bearer ${access_token}`,
